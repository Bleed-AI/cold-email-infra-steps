...
const FOLLOWUPS = [
  ...
  { tag: "SMS", text: "Day 5 · short nudge to the direct mobile" },
];
...
export default function CopyScreen({ businessName, deckHandleRef, onDone }: ScreenProps) {
  ...
  const steps: NarrationStep[] = useMemo(
    () => [
      ...
      { n: "03", title: "Merge both into the copy", detail: <p>Data variables and AI variables slot into a proven template, so every message — email, LinkedIn or SMS — reads like it was written by hand for that one person.</p> },
      ...
      { n: "05", title: "Cross-channel follow-ups", detail: <p>Each angle becomes a short sequence over the next week — email, then a LinkedIn touch, then an SMS nudge — every touch adding something new, so {businessName} stays top-of-mind without nagging.</p> },
    ],
    [businessName]
  );
